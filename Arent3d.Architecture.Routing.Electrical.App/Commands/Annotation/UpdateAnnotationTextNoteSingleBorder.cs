...
namespace Arent3d.Architecture.Routing.Electrical.App.Commands.Annotation
{
  ...
  [DisplayNameKey( "Electrical.App.Commands.Shaft.PlaceTextNoteSingleBorder", DefaultString = "Note\nSingle Border" )]
  [Image( "resources/Initialize-16.bmp", ImageType = ImageType.Normal )]
  [Image( "resources/Initialize-32.bmp", ImageType = ImageType.Large )]
  public class UpdateAnnotationTextNoteSingleBorder : IExternalCommand
  {
    public Result Execute( ExternalCommandData commandData, ref string message, ElementSet elements )
    {
      ...
      ArentTextNote.Clicked = true ;
      var addId = commandData.Application.ActiveAddInId ;
      CheckAndUpdateStorageLines( doc ) ;
      var textNoteUpdater = new TextNoteUpdaterChanged( addId, false ) ;
      var createUpdater = new TextNoteUpdaterCreated( addId, arentTextNoteType, false ) ;
      var viewUpdater = new ViewUpdater( addId, false ) ;
    ...
    private static TextNoteType CheckExistedOrCreateTextNoteArent( Document doc )
    {
      ...
      var newTextNoteType =
        ( (TextNoteType) list.First() ).Duplicate( ArentTextNote.ArentTextNoteType ) as TextNoteType ;
      //newTextNoteType.get_Parameter("Text Font").Set("Arial Narrow");
      tran.Commit() ;
      return newTextNoteType! ;
    }

    private static XYZ GetMiddlePoint( CurveElement line )
    {
    ...
    {
      var allText = new FilteredElementCollector( document, document.ActiveView.Id ).WhereElementIsNotElementType()
        .OfClass( typeof( TextNote ) ).Cast<TextNote>().ToList() ;
      var allLines = new FilteredElementCollector( document, document.ActiveView.Id ).WhereElementIsNotElementType()
        .OfClass( typeof( CurveElement ) ).Where( x => x is DetailLine ).Cast<DetailLine>().ToList() ;
