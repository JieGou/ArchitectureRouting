...
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
  public abstract class NewRackCommandBase : IExternalCommand
  {
    ...
    public static IReadOnlyDictionary<byte, string> RackTypes { get ; } =
      new Dictionary<byte, string> { { 0, "Normal Rack" }, { 1, "Limit Rack" } } ;
        ...
        var result = document.Transaction(
            ...
            "Create Cable Rack For All Route" ), _ =>
          {
            ...
            var routeNames = document.GetAllElements<Element>().OfCategory( BuiltInCategorySets.Conduits )
              .OfNotElementType().Where( filter ).OfType<Element>().Select( x => x.GetRouteName() ).Distinct() ;
    ...
    private void CreateCableRackForRoute( UIDocument uiDocument, Application app, string? routeName,
    ...
    public static void CreateRackForConduit( UIDocument uiDocument, Application app,
      IEnumerable<Element> allElementsInRoute, List<FamilyInstance> racks )
    {
      var document = uiDocument.Document ;
      ...
      double cableRackWidth = 0 )
    {
      ...
      var symbol = document.GetFamilySymbols( ElectricalRoutingFamilyType.CableTray ).FirstOrDefault() ??
                   throw new InvalidOperationException() ; // TODO may change in the future

      // Create cable tray
      if ( false == symbol.IsActive ) symbol.Activate() ;
      var instance = document.Create.NewFamilyInstance(
        new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y, line.Origin.Z ), symbol, null,
        StructuralType.NonStructural ) ;

      // set cable rack length
      SetParameter( instance, "Revit.Property.Builtin.TrayLength".GetDocumentStringByKeyOrDefault( document, "トレイ長さ" ),
      ...
      SetParameter( instance,
        "Revit.Property.Builtin.RackType".GetDocumentStringByKeyOrDefault( document, "Rack Type" ),
        cableRackWidth == 0 ? RackTypes[ 0 ] : RackTypes[ 1 ] ) ;
      ...
      if ( ! string.IsNullOrEmpty( toConnectorId ) )
        SetParameter( instance,
          "Revit.Property.Builtin.ToSideConnectorId".GetDocumentStringByKeyOrDefault( document,
            "To-Side Connector Id" ), toConnectorId ) ;
      if ( ! string.IsNullOrEmpty( fromConnectorId ) )
        SetParameter( instance,
          "Revit.Property.Builtin.FromSideConnectorId".GetDocumentStringByKeyOrDefault( document,
            "From-Side Connector Id" ), fromConnectorId ) ;
      ...
      if ( 1.0 == line.Direction.Y ) {
        ElementTransformUtils.RotateElement( document, instance.Id,
          Line.CreateBound( new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y, firstConnector.Origin.Z ),
            new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y, firstConnector.Origin.Z + 1 ) ), Math.PI / 2 ) ;
      }
        ...
        ElementTransformUtils.RotateElement( document, instance.Id,
          Line.CreateBound( new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y, firstConnector.Origin.Z ),
            new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y - 1, firstConnector.Origin.Z ) ), Math.PI / 2 ) ;
      }
      else if ( -1.0 == line.Direction.Z ) {
        ElementTransformUtils.RotateElement( document, instance.Id,
          Line.CreateBound( new XYZ( firstConnector.Origin.X, firstConnector.Origin.Y, firstConnector.Origin.Z ),
      ...
      if ( 1.0 == line.Direction.Z || -1.0 == line.Direction.Z ) {
        ...
        instance.Location.Move( new XYZ( 0, 0,
          -30d.MillimetersToRevitUnits() ) ) ; // TODO may be must change when FamilyType change
      }

      return instance ;
    }

    public static FamilyInstance CreateRackForFittingConduit( UIDocument uiDocument, FamilyInstance conduit,
      LocationPoint location, double cableTrayDefaultBendRadius = 0 )
    {
      var document = uiDocument.Document ;
      ...
      var length = conduit.ParametersMap
        ...
        .AsDouble() ;
      var diameter = conduit.ParametersMap
        .get_Item( "Revit.Property.Builtin.NominalDiameter".GetDocumentStringByKeyOrDefault( document, "継手外径" ) )
        .AsDouble() ;
      ...
      var symbol =
        uiDocument.Document.GetFamilySymbols( ElectricalRoutingFamilyType.CableTrayFitting ).FirstOrDefault() ??
        throw new InvalidOperationException() ; // TODO may change in the future
      ...
      bendRadius = cableTrayDefaultBendRadius == 0
        ? ( RATIO_BEND_RADIUS * diameter.RevitUnitsToMillimeters() + BendRadiusSettingForStandardFamilyType )
        .MillimetersToRevitUnits()
        : cableTrayDefaultBendRadius ;
      SetParameter( instance,
        "Revit.Property.Builtin.BendRadius".GetDocumentStringByKeyOrDefault( document, "Bend Radius" ),
        bendRadius ) ; // TODO may be must change when FamilyType change
      ...
      SetParameter( instance,
        "Revit.Property.Builtin.RackType".GetDocumentStringByKeyOrDefault( document, "Rack Type" ),
        cableTrayDefaultBendRadius == 0 ? RackTypes[ 0 ] : RackTypes[ 1 ] ) ;
    ...
    {
      var rackNotationStorable =
        doc.GetAllStorables<RackNotationStorable>().FirstOrDefault() ?? doc.GetRackNotationStorable() ;
      RemoveNotationUnused( doc, rackNotationStorable ) ;
      Dictionary<string, Dictionary<double, List<FamilyInstance>>> directionXRacks =
        new Dictionary<string, Dictionary<double, List<FamilyInstance>>>() ;
      Dictionary<string, Dictionary<double, List<FamilyInstance>>> directionYRacks =
        new Dictionary<string, Dictionary<double, List<FamilyInstance>>>() ;
      foreach ( var rack in racks ) {
        var widthRack =
          Math.Round(
            rack.ParametersMap
              .get_Item( "Revit.Property.Builtin.TrayWidth".GetDocumentStringByKeyOrDefault( doc, "トレイ幅" ) ).AsDouble(),
            4 ) ;
        ...
        if ( rack.HandOrientation.X is 1.0 or -1.0 ) {
          if ( directionXRacks.ContainsKey( fromConnectorId ) ) {
            ...
            Dictionary<double, List<FamilyInstance>> xRacks =
              new Dictionary<double, List<FamilyInstance>> { { widthRack, new List<FamilyInstance>() { rack } } } ;
    ...
    {
      var fromElementId = rack.ParametersMap
        .get_Item( "Revit.Property.Builtin.FromSideConnectorId".GetDocumentStringByKeyOrDefault( doc,
          "From-Side Connector Id" ) ).AsString() ;
      if ( string.IsNullOrEmpty( fromElementId ) ) return string.Empty ;
      var fromConnector = doc.GetAllElements<Element>().OfCategory( BuiltInCategorySets.PickUpElements )
    ...
    private static void CreateNotation( Document doc, RackNotationStorable rackNotationStorable,
      IReadOnlyCollection<FamilyInstance> racks, string fromConnectorId, bool isDirectionX, double scale )
    {
      const string xSymbol = " x " ;
      var count = racks.Count ;
      var rack = racks.OrderByDescending( x =>
        x.ParametersMap.get_Item( "Revit.Property.Builtin.TrayLength".GetDocumentStringByKeyOrDefault( doc, "トレイ長さ" ) )
          .AsDouble() ).FirstOrDefault() ;
      if ( rack != null ) {
        var widthCableTray = rack.ParametersMap
          .get_Item( "Revit.Property.Builtin.TrayWidth".GetDocumentStringByKeyOrDefault( doc, "トレイ幅" ) ).AsDouble() ;
        var notationModel = rackNotationStorable.RackNotationModelData.FirstOrDefault( n =>
          n.FromConnectorId == fromConnectorId && n.IsDirectionX == isDirectionX &&
          Math.Abs( n.RackWidth - Math.Round( widthCableTray, 4 ) ) == 0 ) ;
        if ( notationModel == null ) {
          if ( doc.ActiveView is ViewPlan viewPlan ) {
            ...
            point = new XYZ( 0.5 * ( connectors[ 0 ].Origin.X + connectors[ 1 ].Origin.X ),
              0.5 * ( connectors[ 0 ].Origin.Y + connectors[ 1 ].Origin.Y ), point.Z ) ;
            ...
            if ( isDirectionX ) {
              var vector = ( XYZ.BasisX * heightText * multiple + XYZ.BasisY * heightText * multiple +
                             XYZ.BasisY * heightText ) * scale ;
              var transform = Transform.CreateTranslation( vector ) ;
              textNote = TextNote.Create( doc, doc.ActiveView.Id, transform.OfPoint( point ), notation,
                textNoteType.Id ) ;
            }
            else {
              var vector = ( XYZ.BasisX.Negate() * heightText * multiple + XYZ.BasisY * heightText * multiple +
                             XYZ.BasisY * heightText ) * scale ;
              var transform = Transform.CreateTranslation( vector ) ;
              textNote = TextNote.Create( doc, doc.ActiveView.Id, transform.OfPoint( point ), notation,
                textNoteType.Id ) ;
              ElementTransformUtils.MirrorElements( doc, new List<ElementId> { textNote.Id },
                Plane.CreateByNormalAndOrigin( XYZ.BasisX, textNote.Coord ), false ) ;
            }
            ...
            var nearestPoint =
              underLineTextNote.GetEndPoint( 0 ).DistanceTo( point ) >
              underLineTextNote.GetEndPoint( 1 ).DistanceTo( point )
                ? underLineTextNote.GetEndPoint( 1 )
                : underLineTextNote.GetEndPoint( 0 ) ;
            var curves = GeometryHelper.GetCurvesAfterIntersection( viewPlan,
              new List<Curve>
              {
                Line.CreateBound( nearestPoint, new XYZ( point.X, point.Y, viewPlan.GenLevel.Elevation ) )
              }, new List<Type> { typeof( CableTray ) } ) ;
            ...
            (string? endLineUniqueId, int? endPoint) endLineLeader = ( curveClosestPoint.DetailCurve?.UniqueId,
              endPoint: curveClosestPoint.EndPoint ) ;
            var ortherLineId = detailCurves.Select( x => x.UniqueId ).Where( x => x != endLineLeader.endLineUniqueId )
              .ToList() ;

            foreach ( var item in racks ) {
              var rackNotationModel = new RackNotationModel( item.UniqueId, textNote.UniqueId, rack.UniqueId,
                fromConnectorId, isDirectionX, Math.Round( widthCableTray, 4 ), endLineLeader.endLineUniqueId,
                endLineLeader.endPoint, ortherLineId ) ;
              rackNotationStorable.RackNotationModelData.Add( rackNotationModel ) ;
            }
          }
        }
        else {
          var textElement = doc.GetAllElements<Element>().OfCategory( BuiltInCategory.OST_TextNotes )
            .FirstOrDefault( t => t.UniqueId == notationModel.NotationId ) ;
          ...
          foreach ( var item in racks ) {
            var rackNotationModel = new RackNotationModel( item.UniqueId, notationModel.NotationId,
              notationModel.RackNotationId, fromConnectorId, isDirectionX, notationModel.RackWidth,
    ...
    public static Line CreateUnderLineText( TextNote textNote, double elevation )
    {
      ...
      coord = Transform.CreateTranslation( ( textNote.HorizontalAlignment == HorizontalTextAlignment.Right ? 1 : -1 ) *
                                           offset * textNote.Document.ActiveView.Scale * textNote.BaseDirection )
        .OfPoint( coord ) ;
      var width = ( textNote.HorizontalAlignment == HorizontalTextAlignment.Right ? -1 : 1 ) *
                  ( textNote.Width + 2 * offset ) * textNote.Document.ActiveView.Scale ;
    ...
    private static void RemoveNotationUnused( Document doc, RackNotationStorable rackNotationStorable )
    {
      ...
      foreach ( var notationModel in rackNotationStorable.RackNotationModelData ) {
        var rack = doc.GetAllElements<Element>().OfCategory( BuiltInCategorySets.RackTypeElements )
          .FirstOrDefault( c => c.UniqueId == notationModel.RackId ) ;
        if ( rack != null ) continue ;
        notationUnused.Add( notationModel ) ;
        var textElement = doc.GetAllElements<Element>().OfCategory( BuiltInCategory.OST_TextNotes )
          .FirstOrDefault( e => e.UniqueId == notationModel.NotationId ) ;
        if ( textElement == null ) continue ;
        var textNote = textElement as TextNote ;
        var text = textNote!.Text ;
        if ( text.Contains( XChar ) ) {
          var number = text.Substring( text.IndexOf( XChar ) + 1 ).Trim( '\r' ) ;
          textNote.Text = Convert.ToInt16( number ) - 1 == 1
            ? text.Substring( 0, text.IndexOf( XChar ) - 1 )
            : text.Substring( 0, text.IndexOf( XChar ) + 2 ) + ( Convert.ToInt16( number ) - 1 ) ;
        }
        else {
    ...
    private static void CreateNewTextNoteType( Document doc, TextNote textNote )
    {
      ...
      var textNoteType = new FilteredElementCollector( doc ).OfClass( typeof( TextNoteType ) )
        .WhereElementIsElementType().Cast<TextNoteType>().FirstOrDefault( tt => Equals( strStyleName, tt.Name ) ) ;
