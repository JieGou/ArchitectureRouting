...
namespace Arent3d.Architecture.Routing.AppBase.Commands.Routing
{
  public abstract class NewLimitRackCommandBase : IExternalCommand
  {
    ...
    private IEnumerable<FamilyInstance> ConnectedRacks( Document document, List<FamilyInstance> cableTrays, List<FamilyInstance> fittings )
    {
      ...
      if ( ! IsCircle ) {
        var informationCableTrays = ExtendCurves( document, infoCableTrays, fittings ) ;
        var curveLoops = GroupCurves( informationCableTrays ).Select( x => CurveLoop.CreateViaThicken( x.CurveLoop, cableTrayWidth, XYZ.BasisZ ) ) ;
    ...
    private static Line IntersectFitting( Line locationCableTray, IEnumerable<FamilyInstance> fittings, double tolerance )
    {
