    ...
    public abstract class NewRackFromToCommandBase : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                var result = document.Transaction("TransactionName.Commands.Rack.NewRackFromTo".GetAppStringByKeyOrDefault("New Rack From-To"), _ =>
                {
                    ...
                    foreach (var connector in connectors)
                    {
                        ...
                        else // element is conduit fitting
                        {
                            ...
                            {
                                instance.Location.Rotate(Line.CreateBound(new XYZ(location.Point.X, location.Point.Y, location.Point.Z),
                                    new XYZ(location.Point.X, location.Point.Y, location.Point.Z - 1)),
                                    Math.PI / 2);
                            }
                    ...
                    foreach (Connector connector in rackConnectors)
                    {
                        ...
                        {
                            var otherConnectors = rackConnectors.FindAll(x => !x.IsConnected && x.Owner.Id != connector.Owner.Id);
        ...
        private static Connector? GetConnectorClosestTo(
          ...
          XYZ point, double maxDistance = double.MaxValue)
        {
        ...
        /// <param name="connectors"></param>
        /// <returns></returns>
        private static Connector? GetFirstConnector(
        ...
        /// <param name="connectors"></param>
        /// <returns></returns>
        private static Connector? GetLastConnector(
          ConnectorSet connectors)
        {
            int maxId = -1;
            Connector? targetConnector = null;
        ...
        private static IEnumerable<Connector> FindAllConnectorsBetweenTwoElement(Element fromElement, Element toElement)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                {
                    neighbours.AddRange(connecToConnector.GetOtherConnectorsInOwner().Where(x => !visited.Any(y => y.Owner.Id == x.Owner.Id && y.Id == x.Id)));
                }
