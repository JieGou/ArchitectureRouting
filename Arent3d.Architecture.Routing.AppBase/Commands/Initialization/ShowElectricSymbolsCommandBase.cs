...
namespace Arent3d.Architecture.Routing.AppBase.Commands.Initialization
{
  public class ShowElectricSymbolsCommandBase : IExternalCommand
  {
    public Result Execute( ExternalCommandData commandData, ref string message, ElementSet elements )
    {
      ...
      foreach ( var element in pickedObjects ) {
        ...
        if ( ! string.IsNullOrEmpty( existSymbolRoute ) && ceedStorable != null ) {
          if ( ! processedDetailSymbol.Contains( existSymbolRoute ) ) {
            ...
            var (startCeeDSymbol, startCondition, endCeeDSymbol, endCondition) = GetFromConnectorAndToConnectorCeeDCode( doc, startTeminateId, endTeminateId ) ;
            ...
            var startElectricalSymbolModel = new ElectricalSymbolModel( startCeeDModel?.FloorPlanSymbol ?? string.Empty, startCeeDModel?.GeneralDisplayDeviceSymbol ?? string.Empty, string.Empty, string.Empty, string.Empty, pipingType, string.Empty ) ;
            var endElectricalSymbolModel = new ElectricalSymbolModel( endCeeDModel?.FloorPlanSymbol ?? string.Empty, endCeeDModel?.GeneralDisplayDeviceSymbol ?? string.Empty, string.Empty, string.Empty, string.Empty, pipingType, string.Empty ) ;
      ...
      {
        TextNote.Create( doc, doc.ActiveView.Id, txtPosition, noteWidth, GenerateTextTable( electricalSymbolModels ), opts ) ;
    ...
    private static (string, string, string, string) GetFromConnectorAndToConnectorCeeDCode( Document document, string fromElementId, string toElementId )
    {
      ...
      return ( fromGeneralSymbol, fromCondition, toGeneralSymbol, toCondition ) ;
    }
