    ...
    public virtual void RenderScene( View view, DisplayStyle displayStyle )
    {
      try {
        ...
        if ( displayStyle != DisplayStyle.Shading && this._edgeBufferStorage.PrimitiveCount > 0 ) {
          DrawContext.FlushBuffer( this._edgeBufferStorage.VertexBuffer, this._edgeBufferStorage.VertexBufferCount, this._edgeBufferStorage.IndexBuffer, this._edgeBufferStorage.IndexBufferCount, this._edgeBufferStorage.VertexFormat,
            this._edgeBufferStorage.EffectInstance, PrimitiveType.LineList, 0, this._edgeBufferStorage.PrimitiveCount ) ;
        }
      }
      catch ( Exception ) {
        //
      ...
      bufferStorage.IndexBuffer.Map( indexBufferSizeInShortInts ) ;
      {
        ...
        foreach ( var xyzs in edges ) {
          ...
          for ( var i = 1 ; i < xyzs.Count ; i++ ) {
            ...
            indexStream.AddLine( new IndexLine( startIndex + i - 1, startIndex + i ) ) ;
          }
